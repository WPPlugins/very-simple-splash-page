...
class VSSP_SplashPage
{
	...
	{
		$this->settings = get_option( 'vssp_options' );

		$this->config	= get_option( 'vssp_settings' );
	...
	function splash_page() {
		...
		if (isset($this->settings['enable_vs']) && $this->settings['enable_vs'] == 1) {
			
				setCookie( $this->cookie_name, time(), $this->cookie_expiration );
				require_once (VS_SPLASH_PAGE_ROOT_PATH.'templates/default-template.php');
				exit;
